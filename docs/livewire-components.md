        ...
        <div wire:loading.remove>
            ...
            @else
                <p class="text-gray-500 text-sm">
                    Nenhum efetivo da OM encontrado neste BCA.
                </p>
            @endif
        </div>
    @endif
...
class ListagemEfetivo extends Component
{
    ...
    #[Url]  // Persiste o filtro na URL (?busca=SILVA)
    public string $busca = '';
...
<head>
    ...
    <title>{{ config('app.name', 'BCA Scrap') }}</title>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16">
                <div class="flex items-center gap-8">
                    <span class="font-bold text-lg">{{ config('app.name', 'BCA Scrap') }}</span>
                    <a href="{{ route('busca') }}"
