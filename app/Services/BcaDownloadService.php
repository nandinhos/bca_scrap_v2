...
class BcaDownloadService
{
    ...
    public function baixarBca(string $data): ?string
    {
        ...
        if ($cached === 'nao_encontrado') {
            Log::info("BCA [{$data}]: cache hit (não encontrado)");
            // Clear cache to allow re-trial with new logic
            Cache::forget($cacheKey);
        }

        // Search CENDOC using correct intraer method
        $url = $this->buscarUrlBca($data);

        if ($url === null) {
            Cache::put($cacheKey, 'nao_encontrado', now()->addHour());
            return null;
        }
