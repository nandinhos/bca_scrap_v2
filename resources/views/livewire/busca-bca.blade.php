...
{{-- Toggle prévia --}}
            ...
            @if(count($keywordsEncontradas) > 0)
            <div style="margin-top:16px">
                ...
                @foreach($keywordsEncontradas as $palavra => $hit)
                <div x-data="{ open: false }" style="background:white;border-radius:10px;border:1px solid #e2e8f0;margin-bottom:6px;overflow:hidden">
                    <div style="padding:12px 16px;display:flex;align-items:center;gap:12px">
                        ...
                        @if(is_array($hit) && !empty($hit['snippet']))
    ...
    <div style="position:fixed;bottom:0;left:0;width:100%;background:#1e293b;color:#94a3b8;padding:10px 0;text-align:center;font-size:12px;font-weight:500;z-index:100">
        {{ config('app.name', 'BCA Scrap') }} · Força Aérea Brasileira
    </div>
</div>
