...
class HistoricoOcorrencias extends Component
{
    ...
    public function updatedFiltroMilitar(): void { $this->resetPage(); }

    public function render()
    {
        $query = BcaOcorrencia::with(['efetivo','bca'])->orderByDesc('created_at');
        if ($this->filtroData) $query->whereHas('bca', fn($q) => $q->whereDate('data', $this->filtroData));
        if ($this->filtroMilitar) $query->whereHas('efetivo', fn($q) => $q->where('nome_guerra','ilike',"%{$this->filtroMilitar}%")->orWhere('nome_completo','ilike',"%{$this->filtroMilitar}%"));
